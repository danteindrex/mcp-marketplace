...
async function ensureBridgeBinary() {
  ...
  const outputPath = path.join(outputDir, 'mcp-local-bridge-macos')
  ...
  try {
    await execFileAsync(
      'go',
      ['build', '-o', outputPath, './cmd/local-bridge'],
      {
        cwd: backendRoot,
        windowsHide: true,
        env: {
    ...
    throw new Error(`Unable to build MCP Local Bridge for macOS: ${message}`)
  }
  ...
  try {
    ...
    return new NextResponse(binary, {
      ...
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="mcp-local-bridge-macos"',
